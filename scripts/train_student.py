...
def main():
    ...
    run = wandb.init(
        project="mlp-project",
        name=f"{args.machine}_{model_name}_distill-{use_teacher}_alpha{alpha}_beta{beta}_T{temperature}_{optimizer_name}",
        config={
    ...
    if use_teacher and teacher_df is not None:
        ...
        teacher_entry = teachers_cfg[teacher_df]
    ...
    loss_csv_path = f'{save_dir}/{model_name}_loss_curves.csv'
    ...
    print(f"Distill:     alpha={alpha}, beta={beta}, T={temperature}")
